...
export function normalizeHouseholdNumber(value: unknown): string {
  return String(value ?? '').normalize('NFKC').trim().replace(/\s*호\s*$/u, '').replace(/^0+(?=\d)/u, '');
}
...
export async function loadHouseholdVerificationConfig(fetchImpl: typeof fetch = fetch): Promise<HouseholdVerificationConfigV1> {
  ...
  try {
    return parseHouseholdVerificationConfig(await response.json());
  } catch (error) {
...
export async function verifyHousehold(
  ...
  try {
    ...
    if (controller.signal.aborted || error instanceof DOMException && error.name === 'AbortError') {
      throw new HouseholdVerificationError('timeout');
    }
    throw new HouseholdVerificationError('network');
  } finally {
    clearTimeout(timer);
...
export function readHouseholdVerificationSession(storage: SessionStoragePort | null = browserSessionStorage()): HouseholdVerificationSessionV1 | null {
  ...
  try {
    ...
    if (!parsed
      ...
      || parsed.provider !== HOUSEHOLD_VERIFICATION_PROVIDER
      || typeof parsed.verifiedAt !== 'number'
      || !Number.isFinite(parsed.verifiedAt)
      || parsed.verifiedAt <= 0) return null;
    return {
      schemaVersion: 1,
      provider: HOUSEHOLD_VERIFICATION_PROVIDER,
