...
class CalculatorApp {
  ...
  private scheduleTable(rows: ScheduleRow[]): string {
    ...
    return `<div class="schedule-scroll"><table class="schedule-table"><thead><tr><th>회차</th><th>납부일</th><th>주택</th><th>발코니</th><th>시스템<br>에어컨</th><th>기타 옵션</th><th>회차 합계</th></tr></thead><tbody>${rows.map((row) => { const total = row.supply + row.balcony + row.systemAc + row.other; const phase = row.label === '계약금' ? 'contract' : row.label === '잔금' ? 'balance' : 'interim'; return `<tr data-phase="${phase}"><th><strong>${escape(row.label)}</strong></th><td class="schedule-date">${escape(row.due)}</td><td>${cell(row.supply)}</td><td>${cell(row.balcony)}</td><td>${cell(row.systemAc)}</td><td>${cell(row.other)}</td><td class="total">${money(total)}</td></tr>`; }).join('')}</tbody></table></div>`;
  }
  ...
  private taxDetails(quote: Quote): string {
    ...
    const result = quote.tax ? `<dl><dt>과세 추정 기준</dt><dd>${money(quote.tax.taxable)}</dd><dt>적용 취득세율</dt><dd>${quote.tax.ratePercent}%</dd><dt>선택 감면 반영</dt><dd>−${money(quote.tax.relief)}</dd><dt>취득세</dt><dd>${money(quote.tax.acquisition)}</dd><dt>지방교육세</dt><dd>${money(quote.tax.education)}</dd><dt>농어촌특별세</dt><dd>${money(quote.tax.rural)}</dd><dt class="calculation-total-label">합계</dt><dd class="calculation-total">${money(quote.tax.total)}</dd></dl>` : '<p class="muted-copy">활성화 시 계약 예상 총액 기준으로 계산합니다.</p>';
    return `<details id="calculator-tax-details" class="calculator-card disclosure" ${this.taxEnabled ? 'open' : ''}><summary>취득세 추정 <span>펼치기</span></summary><div class="disclosure-body"><label class="enable-row"><input id="calculator-tax-enabled" type="checkbox" ${this.taxEnabled ? 'checked' : ''}> 취득세 추정 활성화</label><fieldset class="relief-options" ${this.taxEnabled ? '' : 'disabled'}><legend>감면 시나리오</legend>${choices.map(([value, label, amount]) => `<label><input name="tax-relief-mode" type="radio" value="${value}" ${this.taxReliefMode === value ? 'checked' : ''}><span><b>${label}</b><small>${amount ? `최대 ${money(amount)}` : '기본 계산'}</small></span></label>`).join('')}</fieldset>${result}<p class="legal-copy">생애최초는 현행법상 최대 200만원, 출산·양육 감면은 최대 500만원 시나리오입니다. 입주 예정 시점에는 일몰·자격·1주택 요건이 달라질 수 있어 자동 확정하지 않습니다. 지방교육세는 감면 전 취득세 기준의 단순 추정이며 등기·법무·대출 부대비용은 제외됩니다. <a href="https://www.law.go.kr/법령/지방세특례제한법/제36조의3" target="_blank" rel="noreferrer">생애최초 조문 ↗</a> · <a href="https://www.law.go.kr/법령/지방세특례제한법/제36조의5" target="_blank" rel="noreferrer">출산·양육 조문 ↗</a></p></div></details>`;
  }

  private interimLoanDetails(quote: Quote): string {
    const details = quote.interimLoan;
    const interimRows = quote.schedule.filter((row) => row.label.includes('중도금'));
    const selfFunded = interimRows.reduce((total, row) => total + row.balcony + row.systemAc + row.other, 0);
    const balconyInterim = interimRows.reduce((total, row) => total + row.balcony, 0);
    const systemAcInterim = interimRows.reduce((total, row) => total + row.systemAc, 0);
    const otherInterim = interimRows.reduce((total, row) => total + row.other, 0);
    const result = this.interimLoanEnabled ? `<dl><dt>은행 집단대출 원금</dt><dd>${money(details.totalPrincipalWon)}</dd><dt class="calculation-total-label">입주 시 납부 예상 이자</dt><dd class="calculation-total">${money(details.totalInterestWon)}</dd><dt>발코니 중도금 · 자납</dt><dd>${money(balconyInterim)}</dd><dt>시스템에어컨 중도금 · 자납</dt><dd>${money(systemAcInterim)}</dd><dt>기타 옵션 중도금 · 자납</dt><dd>${money(otherInterim)}</dd><dt>계산상 입주 기준일</dt><dd>${details.assumedMoveInDate}</dd></dl><ul class="tranche-list">${details.tranches.map((tranche, index) => `<li><span>${this.plan === 'main-subscription' ? `${index + 1}차 · ` : ''}${tranche.executionDate}<small>${tranche.days}일 후불</small></span><b>${money(tranche.principalWon)}<small>이자 ${money(tranche.interestWon)}</small></b></li>`).join('')}</ul>` : '<p class="muted-copy">활성화하면 주택 공급대금 중도금 20%에 대한 이자후불 예상액을 계산합니다.</p>';
    return `<details id="calculator-interim-loan-details" class="calculator-card disclosure" ${this.interimLoanEnabled ? 'open' : ''}><summary>중도금 대출 이자 추정 <span>펼치기</span></summary><div class="disclosure-body"><div class="loan-scope"><span><small>집단대출 추정 대상</small><b>주택 공급대금 중도금 20%</b></span><span><small>별도 현금납부 예상</small><b>${money(selfFunded)}</b></span></div><label class="enable-row"><input id="calculator-interim-loan-enabled" type="checkbox" ${this.interimLoanEnabled ? 'checked' : ''}> 이자후불제 추정 활성화</label><div class="rate-stepper" aria-label="예상 연 금리"><span>예상 연 금리</span><div><button id="calculator-interim-rate-down" type="button" ${this.interimLoanEnabled ? '' : 'disabled'} aria-label="금리 0.5%포인트 낮추기">−</button><strong>${this.interimRatePercent.toFixed(1)}%</strong><button id="calculator-interim-rate-up" type="button" ${this.interimLoanEnabled ? '' : 'disabled'} aria-label="금리 0.5%포인트 높이기">＋</button></div></div>${result}<p class="legal-copy">공고문은 중도금 융자알선을 <b>전체 공급대금의 20% 범위</b>로 명시합니다. 발코니·시스템에어컨·기타 옵션에는 계약상 중도금 회차가 있지만 별도 계약·계좌이며, 은행 집단대출 포함 근거가 없어 자납으로 분리했습니다. 금융기관·금리는 아직 미정이고 4.0%는 계획용 기본값입니다. 2029-02-01을 계산 경계로 사용합니다.</p></div></details>`;
  }

  private mortgageDetails(quote: Quote): string {
    return `<details id="calculator-mortgage-details" class="calculator-card disclosure" ${this.mortgageEnabled ? 'open' : ''}><summary>신혼희망타운 전용 모기지 추정 <span>열기</span></summary><div class="disclosure-body"><label class="enable-row"><input id="calculator-mortgage-enabled" type="checkbox" ${this.mortgageEnabled ? 'checked' : ''}> 대출 추정 활성화</label><div class="mini-controls mortgage-controls"><label>LTV<select id="calculator-ltv" ${this.mortgageEnabled ? '' : 'disabled'}>${[30,40,50,60,70].map((value) => `<option ${value === this.ltv ? 'selected' : ''}>${value}</option>`).join('')}</select></label><label>상환 만기<select id="calculator-term" ${this.mortgageEnabled ? '' : 'disabled'}><option value="20" ${this.term === 20 ? 'selected' : ''}>20년</option><option value="30" ${this.term === 30 ? 'selected' : ''}>30년</option></select></label><label>자녀 수<select id="calculator-children" ${this.mortgageEnabled ? '' : 'disabled'}><option value="0" ${this.childCount === 0 ? 'selected' : ''}>0명</option><option value="1" ${this.childCount === 1 ? 'selected' : ''}>1명</option><option value="2" ${this.childCount === 2 ? 'selected' : ''}>2명 이상</option></select></label><label>정산 시 대출 유지기간<select id="calculator-settlement-period" ${this.mortgageEnabled ? '' : 'disabled'}><option value="1-9" ${this.settlementPeriod === '1-9' ? 'selected' : ''}>1~9년</option><option value="14" ${this.settlementPeriod === '14' ? 'selected' : ''}>14년</option><option value="19" ${this.settlementPeriod === '19' ? 'selected' : ''}>19년</option><option value="24+" ${this.settlementPeriod === '24+' ? 'selected' : ''}>24년 이상</option></select></label></div>${quote.mortgage ? `<dl><dt>적용 LTV</dt><dd>${quote.mortgage.ltvPercent}%</dd><dt>공고문 기준 연 이율</dt><dd>${quote.mortgage.annualRatePercent}% 고정</dd><dt>대출 가능 상한</dt><dd>${money(quote.mortgage.eligibleMaximum)}</dd><dt>대출 원금</dt><dd>${money(quote.mortgage.principal)}</dd><dt>입주 시 잔금 자기자금</dt><dd>${money(quote.mortgage.moveInCash)}</dd><dt>1년 거치 월 이자</dt><dd>${money(quote.mortgage.graceInterest)}</dd><dt>거치 후 ${quote.mortgage.repaymentMonths / 12}년 월 원리금</dt><dd>${money(quote.mortgage.repayment)}</dd><dt>향후 매각차익 기금 정산비율</dt><dd>${quote.mortgage.fundSharePercent}%</dd></dl>` : '<p class="muted-copy">활성화 시 공고문 기준 LTV 30~70%, 연 1.3%, 1년 거치 조건으로 추정합니다.</p>'}<p class="legal-copy"><b>상환 만기</b>는 월 원리금 계산에, <b>정산 시 대출 유지기간</b>은 자녀 수와 함께 향후 매각차익의 기금 정산비율에만 반영됩니다. 매각차익 금액은 계산하지 않습니다. 대출액은 조정 공급가×LTV와 400,000,000원 중 작은 금액이며, 발코니·옵션·세금은 대출 기준에서 제외됩니다. 공고문 이후 실행 시점 기금운용계획과 실제 심사 결과가 우선합니다. <a href="https://lh.or.kr/menu.es?mid=a10402010200" target="_blank" rel="noreferrer">LH 안내 ↗</a></p></div></details>`;
  }
