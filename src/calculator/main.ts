...
import { calculateBundangSaleFinance, type BundangPaymentPlanKind, type BundangSaleFloorBand, type BundangSaleUnitType } from '../game/bundang-sale-finance';
...
class CalculatorApp {
  ...
  private mortgageDetails(quote: Quote): string {
    return `<details id="calculator-mortgage-details" class="calculator-card disclosure" ${this.mortgageEnabled ? 'open' : ''}><summary>대출 추정 <span>열기</span></summary><div class="disclosure-body"><label class="enable-row"><input id="calculator-mortgage-enabled" type="checkbox" ${this.mortgageEnabled ? 'checked' : ''}> 대출 추정 활성화</label><div class="mini-controls"><label>LTV<select id="calculator-ltv" ${this.mortgageEnabled ? '' : 'disabled'}>${[30,40,50,60,70].map((value) => `<option ${value === this.ltv ? 'selected' : ''}>${value}</option>`).join('')}</select></label><label>기간<select id="calculator-term" ${this.mortgageEnabled ? '' : 'disabled'}><option value="20" ${this.term === 20 ? 'selected' : ''}>20년</option><option value="30" ${this.term === 30 ? 'selected' : ''}>30년</option></select></label></div>${quote.mortgage ? `<dl><dt>적용 LTV</dt><dd>${quote.mortgage.ltvPercent}%</dd><dt>연 이율</dt><dd>${quote.mortgage.annualRatePercent}% 고정</dd><dt>대출 가능 상한</dt><dd>${money(quote.mortgage.eligibleMaximum)}</dd><dt>대출 원금</dt><dd>${money(quote.mortgage.principal)}</dd><dt>입주 시 자기자금</dt><dd>${money(quote.mortgage.moveInCash)}</dd><dt>1년 거치 월 이자</dt><dd>${money(quote.mortgage.graceInterest)}</dd><dt>거치 후 ${quote.mortgage.repaymentMonths / 12}년 월 원리금</dt><dd>${money(quote.mortgage.repayment)}</dd></dl>` : '<p class="muted-copy">활성화 시 LTV 30~70%, 연 1.6%, 1년 거치 조건으로 추정합니다.</p>'}<p class="legal-copy">대출액은 조정 공급가×LTV와 400,000,000원 중 작은 금액입니다. 수익공유·보증료·인지대·부대비용·실제심사는 계산하지 않습니다. 발코니·옵션·세금은 대출 기준에서 제외되며, 실행 시점 기금운용계획에 따라 달라질 수 있습니다. <a href="https://lh.or.kr/menu.es?mid=a10402010200" target="_blank" rel="noreferrer">LH 안내 ↗</a></p></div></details>`;
  }
  ...
  private bind(): void {
    ...
    this.mount.querySelector<HTMLSelectElement>('#calculator-ltv')?.addEventListener('change', (event) => { this.ltv = Number((event.target as HTMLSelectElement).value); this.render(); });
